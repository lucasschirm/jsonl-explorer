...
/**
 * Worker stand-in that answers RPCs automatically with canned values.
 * Message types listed in `gated` are held until `release()` — used to
 * simulate long-running inits/filters.
 */
class AutoWorker {
  ...
  private gate = new Set<string>()
  private listeners = new Map<string, Set<(event: { data: unknown }) => void>>()

  constructor(private readonly options: { gate?: string[] } = {}) {
    if (options.gate) this.gate = new Set(options.gate)
  }
  ...
  release(): void {
    ...
    this.held = []
    for (const message of pending) this.respond(message as { type?: string; requestId?: string })
  }
  ...
  private respond(msg: { type?: string; requestId?: string; file?: File; name?: string }): void {
    const value =
      msg.type === 'initFile'
        ? { name: msg.file?.name ?? 'a.jsonl', size: 11, type: 'file' }
        : msg.type === 'initUrl'
          ? { name: 'remote.jsonl', size: 22, type: 'url' }
          : msg.type === 'initMemory'
            ? { name: msg.name ?? 'handover.jsonl', size: 33, type: 'handover' }
            : msg.type === 'filter'
